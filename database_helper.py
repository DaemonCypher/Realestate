...
def fetch_from_db(conn, table_name):
    ...
    Returns:
        ...
        Each tuple contains:
            ...
            - status_date (str): The status date value.
            - data_date (str): The data date value.
    """
    cursor = conn.cursor()
    ...
    for row in results:
        id,address, city, history_str,status,status_date,data_date = row
        # Deserialize the history field
        history = json.loads(history_str)
        deserialized_results.append((id, address, city, history,status,status_date,data_date))
...
def addresses_in_db(conn, table_name, addresses):
    """
    Check if addresses from list of addresses are in the database.
